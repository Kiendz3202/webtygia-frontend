...
import { useStockListHnx30 } from '@services/stock/useStock';
import { useRouter } from 'next/router';
import React, { useState } from 'react';

function Hnx30StockView() {
	const [pageNumber, setPageNumber] = useState(1);
	const [exchangeName, setExchangeName] = useState('hnx30');
	const router = useRouter();

	const { data, isLoading, isError, error, isFetching } = useStockListHnx30(
		exchangeName,
		pageNumber
	);
	// console.log(data);
