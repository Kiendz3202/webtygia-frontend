...
import { useStockListHose } from '@services/stock/useStock';
import { useRouter } from 'next/router';
import React, { useState } from 'react';

function HoseStockView() {
	const [pageNumber, setPageNumber] = useState(1);
	const [exchangeName, setExchangeName] = useState('hose');
	const router = useRouter();

	const { data, isLoading, isError, error, isFetching } = useStockListHose(
		exchangeName,
		pageNumber
	);
	console.log(data);
