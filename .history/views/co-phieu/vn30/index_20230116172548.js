...
import { useStockListVn30 } from '@services/stock/useStock';
import { useRouter } from 'next/router';
import React, { useState } from 'react';

function Vn30StockView() {
	const [pageNumber, setPageNumber] = useState(1);
	const [exchangeName, setExchangeName] = useState('vn30');
	const router = useRouter();

	const { data, isLoading, isError, error, isFetching } = useStockListvn30(
		exchangeName,
		pageNumber
	);
	// console.log(data);
