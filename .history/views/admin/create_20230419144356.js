...
function CreateNewsView() {
	...
	const createBlog = async () => {
		...
		try {
			...
			axios({
				method: 'post',
				url: `${process.env.NEXT_PUBLIC_PRODUCT_URL}/create-newss`,
				data: data,
				...
				<div className="mt-16 max-w-[1440px] mx-auto px-[1rem]">
					...
					<div className="my-12">
						{status === 'loading' ? (
							...
							<button
								className={` bg-blue-text w-36 h-12 flex-center text-xl font-semibold text-white rounded-md hover:opacity-80 cursor-pointer duration-100`}
								onClick={() => {
									// if (status === 'idle') {
									createBlog();
									// 	return;
