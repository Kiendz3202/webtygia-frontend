...
const useNewsList = (page) => {
	...
	const fetchNews = async (page) => {
		return axios
			.get(
				`${process.env.NEXT_PUBLIC_BASE_URL}/get-list-news?per_page=1&page=${page}`
			)
...
const useNewsLatest = () => {
	const fetchNews = async () => {
		return axios
			.get(`${process.env.NEXT_PUBLIC_BASE_URL}/get-latest-news`)
			.then((res) => res.data.data)
			.catch((err) => console.log(err));
	};
...
const useNewsdetail = (id) => {
	...
	const fetchNews = async () => {
		return axios
			.get(`${process.env.NEXT_PUBLIC_BASE_URL}/get-detail-news/${id}`)
			.then((res) => res.data.data)
			.catch((err) => console.log(err));
	};
