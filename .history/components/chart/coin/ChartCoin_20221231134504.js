...
import { useCoinChart1D } from '@services/coin/useCoin';

const fetchCoinChart1D = async (nameId) => {
	return axios
		.get(`http://66.42.58.72/api/v1/coin/chart/${nameId}`)
		.then((res) => res.data.data[0].data)
		.catch((err) => console.log(err));
};
const fetchCoinChart90D = async (nameId) => {
	return axios
		.get(`http://66.42.58.72/api/v1/coin/chart/${nameId}?days=90`)
		.then((res) => res.data.data[0].data)
		.catch((err) => console.log(err));
};
const fetchCoinChartMax = async (nameId) => {
	return axios
		.get(`http://66.42.58.72/api/v1/coin/chart/${nameId}?days=max`)
		.then((res) => res.data.data[0].data)
		.catch((err) => console.log(err));
};
// const data1 = [
...
function ChartCoin({ coin }) {
	// const [chartData, setChartData] = useState();
	// const { data, isLoading, isError, error, isFetching } = useQuery(
	// 	['coinChart', coin.nameId],
	// 	() => fetchCoinChart1D(coin.nameId),
	// 	{
	// 		cacheTime: 3600000,
	...
	// 	}
	// );
	const { data, isLoading, isError, error, isFetching } = useCoinChart1D(
		coin.nameId,
		1
	);

	// const changeChartTo90D = async () => {
	// 	const data = await fetchCoinChart90D(coin.nameId);
	// 	let arrayChart = [];
	// 	if (data) {
	...
	// 	}
	// };
	// const changeChartToMax = async () => {
	// 	const data = await fetchCoinChartMax(coin.nameId);
	// 	let arrayChart = [];
	// 	if (data) {
	// 		data.map((result, index) => {
	// 			arrayChart.push({
