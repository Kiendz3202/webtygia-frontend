...
function CoinItem({
	...
	// 	);
	// }, [JSON.parse(localStorage.getItem('followCoins')), typeof window]);

	const saveCoinLocalStorage = () => {};

	const deleteCoinLocalStorage = () => {};
			...
			{user ? (
				...
				<td className="flex items-center leading-[4.5rem] px-[2px] w-[6.2rem]  z-10 text-center">
					...
					{followCoinsLocalStorage?.includes(coin?._id) ? (
						<StarOutlineIcon
							onClick={() => deleteCoinLocalStorage(coin?._id)}
							className=" text-green-500 cursor-pointer hover:opacity-80"
							sx={{
								fontSize: '20px',
								marginRight: '3px',
