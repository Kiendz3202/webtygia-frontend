...
function StockItem({ stock, index, user, saveStocks, deleteStocks }) {
	...
	return (
		...
		>
			<td className="w-[4.8rem]   text-center text-blue-text font-medium flex h-[4.5rem] leading-[4.5rem] justify-between">
				<span className="mr-[1rem]">
					{user?.followStocks?.includes(stock?._id) ? (
						<StarOutlineIcon
							onClick={() => deleteStocks(stock?._id)}
