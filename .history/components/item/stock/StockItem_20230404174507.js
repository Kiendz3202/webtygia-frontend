...
function StockItem({
	...
	useEffect(() => {
		const isSaved = user?.followStocks.map((item) => {
			// if (item?.symbol?.includes(stock?.symbol)) {
			// 	setIsFollow(true);
			// }
			console.log(item?.symbol?.includes(stock?.symbol));
		});
		// console.log(isFollow);
	}, [user]);
	// console.log(user?.followStocks);
	return (
		<tr
			key={stock.name}
