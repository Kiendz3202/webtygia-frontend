			...
			const fetchSearching = async () => {
				try {
					...
					const data = await axios
						.post(`${process.env.NEXT_PUBLIC_PRODUCT_URL}/search`, {
							payload: searchTerm,
						})
						.then((res) => res.data);
	...
	useEffect(() => {
		const checkIfClickedOutside = (e) => {
			...
			if (
				...
				!arrowMenuRef.current.contains(e.target)
			) {
				setIsOpenMenu(false);
			}
		};
			...
			<div className="  w-full max-w-[144rem] my-auto flex justify-between h-[6rem] items-center px-[28px] lg:px-0">
				...
				<div className="flex h-[4rem]">
					...
					>
						...
						{listSearch && searchTerm && (
							<div
								...
								} absolute left-0 top-[6rem] z-[1000] w-[250px] sm:w-[400px] h-fit max-h-[500px] bg-white text-black rounded-[4px] shadow-shadow-custom border border-grey-boder overflow-y-scroll`}
							>
								{listSearch.map((item, index) => (
									<Link key={index} href={item.href}>
										<div className="flex items-center w-full px-[20px] py-[10px] h-auto hover:bg-grey-boder">
											{item.image && (
												<img
													src={item.image}
						...
						{noResult && (
							<div className=" absolute left-0 top-[6rem] z-[1000] w-[250px] sm:w-[400px] h-fit max-h-[500px] bg-white text-black rounded-[4px] shadow-shadow-custom border border-grey-boder overflow-y-scroll">
					...
					<div className="flex relative items-center justify-between bg-white w-auto rounded-[25px] ml-[9px] sm:ml-[15px]">
						...
						<div className="flex items-center justify-end ml-[0.5rem] sm:ml-[2.5rem] mr-[1rem] text-[#868697]">
							{email ? (
								<div className="hidden sm:block text-[1.4rem] font-medium mr-[1rem]">
									{name}
								</div>
							) : (
							...
							<div
								...
								} transition-all ease-linear duration-100 block absolute top-[55px] right-0 z-[1000] text-[14px] w-[25rem] bg-white shadow-2xl rounded-[5px]`}
							>
								<div
									onClick={closeModal}
