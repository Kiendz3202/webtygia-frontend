...
const fetchCoins = async () => {
	return axios.get('http://localhost:5000/api/v1/coin/top-rank').then((res) => res.data).catch((err) => console.log(err))
}

function Coin() {
	// const [coins, setCoins] = useState([]);
	const { data, isLoading,isError,error, isFetching, refetch } = useQuery('coin-top-rank',fetchCoins,{
        cacheTime: Infinity,
